...
const IncomingJobsChart = () => {
  ...
  // Static data for now (can update based on filter in future)
  const data = [
    { date: '09 May', jobs: 10 },
    { date: '10 May', jobs: 18 },
    { date: '11 May', jobs: 23 },
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-2xl mx-auto h-[480px]">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-neutral">Incoming Jobs</h3>
        <div className="flex space-x-2">
          {filters.map((filter) => (
            <button
              key={filter}
              onClick={() => setActiveFilter(filter)}
              className={`capitalize text-xs px-3 py-1  transition ${
                activeFilter === filter
